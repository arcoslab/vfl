...
class VectorField:
    ...
    def normCart(self):
        return VectorField(lambda pos: self.func(pos) /
                           array([self.funcLenght(pos)] * 3 + [1] * 3))
...
class VectorField_test:
    def __init__(self,
                 funcX=lambda pos, param: 0,
                 funcY=lambda pos, param: 0,
                 funcZ=lambda pos, param: 0,
                 param=None):
        param = param or []
    ...
    def getVector(self, pos):
        return self.funcX(pos, self.param), self.funcY(pos,
                                                       self.param), self.funcZ(
                                                           pos, self.param)

    def __add__(self, other):
        paramNum1 = len(self.param)
        paramNew = self.param + other.param
        return VectorField(lambda pos, param: self.funcX(pos,
                                                         param[:paramNum1]) +
                           other.funcX(pos, param[paramNum1:]),
                           lambda pos, param: self.funcY(pos,
                                                         param[:paramNum1]) +
                           other.funcY(pos, param[paramNum1:]),
                           lambda pos, param: self.funcZ(pos,
                                                         param[:paramNum1]) +
                           other.funcZ(pos, param[paramNum1:]), paramNew)
